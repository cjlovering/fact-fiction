...
def _convertSequence(dictionaries, streams, sequenceId, output, unk, annotated):
    ...
    for sampleIndex in range(maxLen):
        ...
        for streamIndex in range(len(tokensPerStream)):
            ...
            if streamIndex == 1: # S1
                onehot = ['0'] * len(dictionaries[streamIndex])
                onehot[value] = '1'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--unk', help='Name fallback symbol for tokens not in dictionary (same for all columns)', default='<UNK>', required=False)
    args = parser.parse_args()

    # creating inputs
    inputs = [sys.stdin]
    if len(args.input) != 0:
        inputs = [open(os.path.join(args.root, i), encoding='utf-8') for i in args.input]
